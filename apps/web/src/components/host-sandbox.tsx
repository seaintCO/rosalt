...
export function HostSandbox({ locale }: { locale: "en" | "es" }) {
  ...
  if (!launched) return <main className="hs6-launch">
    ...
    <form className="hs6-setup-card" onSubmit={launch}>
      ...
      <small>{es ? "VOYNUE es independiente y no está afiliado ni respaldado por Airbnb." : "VOYNUE is independent and is not affiliated with or endorsed by Airbnb."}</small>
    </form>
  </main>;
  ...
  return <main className="hs6-workspace">
    <aside className="hs6-sidebar">
      <Link href={`/${locale}`} className="hs6-brand"><b>VOYNUE</b><small>POWERED BY ALMA</small></Link>
